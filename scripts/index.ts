...
const projectionParams = d3.geoOrthographic()
    .scale(300)
...
filterGlobe.append("feGaussianBlur").attr("stdDeviation", "2.5").attr("result", "coloredBlur");
...
// Wireframe Grid
const gridGlobe = svgGlobe.append("path")
    .datum(graticuleGenerator)
    .attr("class", "graticule")
    .attr("d", geoPathGenerator)
    .style("fill", "none")
    .style("stroke", getComputedStyle(document.documentElement).getPropertyValue('--text-color').trim())
    .style("stroke-width", "0.5")
    .style("stroke-opacity", "0.3");

// Landmasses
let landPathGlobe = svgGlobe.append("path")
    .style("fill", "none")
    .style("stroke", getComputedStyle(document.documentElement).getPropertyValue('--text-color').trim())
...
const globeRouteLayer = svgGlobe.append("g")
...
d3.timer(function () {
    ...
    if (isAutoRotating) {
        ...
        projectionParams.rotate([rot[0] + rotVelocity[0] * dt, rot[1] + rotVelocity[1] * dt]);
    }
    ...
    globeRouteLayer.selectAll("path")
        .data(trailSegments)
        .join("path")
        .style("fill", "none")
        .style("stroke", "#ff00ff")
...
if (supportsTouch) {
    ...
    globeNode.addEventListener('touchmove', function (event) {
        ...
        if (event.touches.length === 1 && touchStartPoint && touchStartRotate) {
            ...
            const minScale = 150;
            const maxScale = 800;
            const newScale = Math.max(minScale, Math.min(maxScale, pinchStartScale * scaleFactor));
...
if (video.autoplay) {
    video.play().catch(() => { });
                ...
                if (lowSrc === fullSrc || video.dataset.upgraded === 'true' || video.dataset.upgrading === 'true') {
return;
                }
...
video.addEventListener('loadedmetadata', function restorePlayback() {
    if (!Number.isNaN(currentTime) && currentTime > 0) {
        video.currentTime = Math.min(currentTime, Math.max(0, video.duration - 0.1));
    }
    if (shouldPlay) {
        video.play().catch(() => { });
    }
...
video.dataset.upgrading = 'false';
        ...
        function setInitialWindowLayout() {
            ...
            profile.style.left = Math.max(12, Math.floor(vw * 0.5) - profileWidth - 16) + 'px';
