...
const geoPathGenerator = d3.geoPath().projection(projectionParams);
const graticuleGenerator = d3.geoGraticule();

// Glow effect
const defsGlobe = svgGlobe.append("defs");
const filterGlobe = defsGlobe.append("filter").attr("id", "glow");
filterGlobe
  .append("feGaussianBlur")
  .attr("stdDeviation", "2.5")
  .attr("result", "coloredBlur");
const feMergeGlobe = filterGlobe.append("feMerge");
feMergeGlobe.append("feMergeNode").attr("in", "coloredBlur");
feMergeGlobe.append("feMergeNode").attr("in", "SourceGraphic");

// Wireframe Grid
const gridGlobe = svgGlobe
  .append("path")
...
d3.json("data/land-110m.json").then((world) => {
  ...
  landPathGlobe.datum(land).attr("d", geoPathGenerator);
});
...
const globeCities = [
  ...
  {
    name: "MANILA 2005 - 2006, 2016",
    coords: [120.9842, 14.5995],
    type: "pink",
  },
  { name: "NYC 2007 - 2012", coords: [-74.006, 40.7128], type: "pink" },
  { name: "CINCINNATI 2014", coords: [-84.512, 39.1031], type: "pink" },
  { name: "TORONTO 2015", coords: [-79.347, 43.651], type: "pink" },
  { name: "SHANGHAI 2017 - 2021", coords: [121.4737, 31.2304], type: "pink" },
  {
    name: "VANCOUVER 2022 - 2024, 2026",
  ...
  {
    name: "WHISTLER - Home Ski Spot",
    coords: [-122.9574, 50.1163],
    type: "gold",
  },
  {
    name: "NISEKO - Craziest Pow for Skiing",
    coords: [140.6874, 42.8048],
    type: "gold",
  },
  {
    name: "PALAWAN - Swimming with Whalesharks",
    coords: [118.7361, 9.8432],
    type: "gold",
  },
  { name: "LONDON - Favorite City", coords: [-0.1278, 51.5074], type: "gold" },
  {
    name: "TAIWAN - First Time Surfing",
    coords: [120.9605, 23.6978],
    type: "gold",
  },
  {
    name: "DOLOMITES - Best Ski Weather",
    coords: [11.7049, 46.5598],
    type: "gold",
  },
  {
    name: "THUNDER RIDGE - First Ski Spot",
    coords: [-73.5818, 41.5081],
    type: "gold",
  },
  {
    name: "KIPAWA - 22 Day Canoe Trip",
    coords: [-78.9667, 46.9667],
    type: "gold",
  },
];

const globeRouteCoords = [
  [-75.6972, 45.4215],
  [79.8612, 6.9271],
  [120.9842, 14.5995],
  [-74.006, 40.7128],
...
// Precalculate dense path points
const detailedCoords = [];
for (let i = 0; i < globeRouteCoords.length - 1; i++) {
  const p1 = globeRouteCoords[i];
  const p2 = globeRouteCoords[i + 1];
...
const globeRouteLayer = svgGlobe
  .append("g")
  .attr("class", "route-trail-layer")
  .style("filter", "url(#glow)");

const cityGroups = svgGlobe
  .selectAll(".city-node")
  .data(globeCities)
  .enter()
  .append("g")
...
cityGroups
  ...
  .on("mousemove", function (event) {
    const tooltip = d3.select("#globe-tooltip");
    const pointer = d3.pointer(event, document.querySelector(".map-container"));
    tooltip
...
function renderGlobe() {
  gridGlobe.attr("d", geoPathGenerator);
  if (landPathGlobe.datum()) landPathGlobe.attr("d", geoPathGenerator);
  globeRouteLayer.selectAll("path").attr("d", geoPathGenerator);

  cityGroups
    .attr("transform", (d) => {
      const p = projectionParams(d.coords);
      return p ? `translate(${p[0]},${p[1]})` : "translate(-100,-100)";
    })
    .style("display", (d) => {
      const center = projectionParams.invert([globewidth / 2, globeheight / 2]);
...
function updateRotationToggleLabel() {
  ...
  rotationToggleBtn.innerText = isAutoRotating
    ? "Pause Rotation"
    : "Play Rotation";
}
...
function isGlobeActive() {
  const mapVisible =
    mapWindow && getComputedStyle(mapWindow).display !== "none";
...
d3.timer(function () {
  ...
  // Auto-rotation
  if (isAutoRotating) {
    const rot = projectionParams.rotate();
    projectionParams.rotate([
      rot[0] + rotVelocity[0] * dt,
      rot[1] + rotVelocity[1] * dt,
    ]);
  }

  // Data-driven path animation
  trailHeadIdx += dt * routeSpeedPerMs;
  if (trailHeadIdx >= detailedCoords.length) trailHeadIdx = 0;
  ...
  globeNode.addEventListener(
    ...
    function (event) {
      if (event.touches.length === 1) {
        ...
        touchStartRotate = projectionParams.rotate();
        ...
        pinchStartScale = projectionParams.scale();
      }
    },
    { passive: false },
  );
  ...
  globeNode.addEventListener(
    ...
    function (event) {
      ...
      if (event.touches.length === 1 && touchStartPoint && touchStartRotate) {
        ...
        const k = 85 / projectionParams.scale();

        projectionParams.rotate([
          touchStartRotate[0] + dx * k,
          touchStartRotate[1] - dy * k,
        ]);
        renderGlobe();
      } else if (
        ...
        const newScale = Math.max(
          minScale,
          Math.min(maxScale, pinchStartScale * scaleFactor),
        );

        projectionParams.scale(newScale);
        renderGlobe();
      }
    },
    { passive: false },
  ...
  svgGlobe.call(
    d3
      ...
      .on("drag", function (event) {
        const rotate = projectionParams.rotate();
        const k = 75 / projectionParams.scale();
        projectionParams.rotate([
          rotate[0] + event.dx * k,
          rotate[1] - event.dy * k,
        ]);
...
svgGlobe.on("wheel", function (event) {
  ...
  const minScale = 150;
  const maxScale = 800;
  let scale = projectionParams.scale();
  scale += event.deltaY * -0.5;
  scale = Math.max(minScale, Math.min(maxScale, scale));
  projectionParams.scale(scale);
  renderGlobe();
});
